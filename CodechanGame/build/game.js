        ...
        var FreeBullet = (function () {
            function FreeBullet(self, player) {
                ...
                for (var i in this.bullets.children) {
                    ...
                    this.bullets.children[i].body.collides(Client.Defined.playerCG, this.hitAndDestroy, this);
    ...
    (function (Client) {
        var Level = (function (_super) {
            ...
            Level.prototype.create = function () {
                ...
                for (var idx in this.mapObjs) {
                    this.game.physics.p2.enable(this.mapObjs[idx]);
                    this.mapObjs[idx].setCollisionGroup(Client.Defined.wallsCG);
                    this.mapObjs[idx].collides(Client.Defined.playerCG);
                    this.mapObjs[idx].collides(Client.Defined.bulletsCG);
                }
                ...
                Client.Defined.connection.onNewPlayer(function (otherPlayer) {
                    _this.game.physics.p2.enable(otherPlayer.player);
                    ...
                    for (var i in otherPlayer.freeBullet.bullets.children) {
                        otherPlayer.freeBullet.bullets.children[i].body.collides(Client.Defined.playerCG);
                        otherPlayer.freeBullet.bullets.children[i].body.collides(Client.Defined.wallsCG);
                    }
                    _this.player.player.body.collides(otherPlayer.freeBullet.freeBulletsCG);
                    for (var idx in _this.mapObjs) {
                        _this.mapObjs[idx].collides(otherPlayer.freeBullet.freeBulletsCG);
                    }
                    for (var i in Client.Defined.playerList) {
                        if (Client.Defined.playerList[i].servID == otherPlayer.servID)
                            continue;
                        Client.Defined.playerList[i].player.body.collides(otherPlayer.freeBullet.freeBulletsCG);
                        for (var x in otherPlayer.freeBullet.bullets.children) {
                            otherPlayer.freeBullet.bullets.children[i].body.collides(Client.Defined.playerList[i].currPlayerCG);
                        }
                    }
                });
